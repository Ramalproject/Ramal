...
export async function GET() {
  ...
  const { data } = await supabase.from('authority_signals').select('*').in('project_id', ids).order('created_at', { ascending: false })
...
export async function POST(req: NextRequest) {
  ...
  const { data } = await supabase.from('authority_signals').insert({ project_id: projectId, ...body }).select('*').single()
  return NextResponse.json(data)
}
...
export async function PATCH(req: NextRequest) {
  ...
  await supabase.from('authority_signals').update(updates).eq('id', id)
...
export async function DELETE(req: NextRequest) {
  ...
  if (id) await supabase.from('authority_signals').delete().eq('id', id)
  return NextResponse.json({ success: true })
}
